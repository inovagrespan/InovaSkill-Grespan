    ...
    <div className="page-shell app-background space-y-6">
      <header className="animate-fade-in flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Visualize clientes pelo endereço geocodificado ou, como fallback, pela cidade cadastrada.</p>
        </div>
      ...
      {!error && withoutCoordinates > 0 && (
        <Alert>
          <AlertDescription>
            {withoutCoordinates} de {totalCustomers} cliente(s) ainda não têm coordenada de endereço nem coordenada municipal resolvida.
          </AlertDescription>
        </Alert>
      )}
