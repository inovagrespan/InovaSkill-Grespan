...
import { Bell, Bot, ChevronLeft, ChevronRight, Clock, DollarSign, Gauge, Search, Save, TrendingDown } from "lucide-react";
...
const formatDuration = (milliseconds: number | null | undefined) => milliseconds == null ? "Sem dados" : milliseconds < 1000 ? `${milliseconds} ms` : `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 2 }).format(milliseconds / 1000)} s`;
const formatPercentage = (value: number | null | undefined) => value == null ? "Sem dados" : `${new Intl.NumberFormat("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value)}%`;
...
function AiConsumptionPage() {
  ...
  return <div className="page-shell space-y-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      {[{ icon: Clock, label: "Tempo médio", value: formatDuration(report?.performance.averageMilliseconds) }, { icon: Clock, label: "Mediana", value: formatDuration(report?.performance.medianMilliseconds) }, { icon: Clock, label: "P95", value: formatDuration(report?.performance.p95Milliseconds) }, { icon: TrendingDown, label: "Redução vs. 10 min", value: formatPercentage(report?.performance.reductionAgainstTenMinutesPercent) }, { icon: TrendingDown, label: "Redução vs. 2 h", value: formatPercentage(report?.performance.reductionAgainstTwoHoursPercent) }].map(({ icon: Icon, label, value }) => <div key={label} className="rounded-xl border border-border bg-surface p-5"><Icon className="size-5 text-primary"/><p className="mt-3 text-xs uppercase text-muted-foreground">{label}</p><p className="mt-1 text-2xl font-semibold">{value}</p></div>)}
    </section>
    ...
    <section className="rounded-xl border border-border bg-surface p-5"><h2 className="font-display text-xl">Amostra das consultas</h2><p className="mt-1 text-sm text-muted-foreground">Até 10 consultas mais recentes dentro dos filtros, vinculadas ao log original.</p><div className="mt-4 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">Pergunta recebida</th><th>Resposta concluída</th><th>Usuário</th><th>Canal</th><th>Pergunta</th><th>Resposta</th><th>Tempo</th><th>Status</th></tr></thead><tbody>{!report || report.querySample.length === 0 ? <tr><td className="p-6 text-center text-muted-foreground" colSpan={8}>Nenhuma consulta no período.</td></tr> : report.querySample.map((item) => <tr key={item.id} className="border-b align-top"><td className="p-2 whitespace-nowrap">{new Date(item.questionReceivedAt).toLocaleString("pt-BR")}</td><td className="whitespace-nowrap">{item.responseCompletedAt ? new Date(item.responseCompletedAt).toLocaleString("pt-BR") : "Não concluída"}</td><td>{item.userName}</td><td>{item.channel === "whatsapp" ? "WhatsApp" : "Web"}</td><td className="max-w-xs whitespace-normal">{item.question}</td><td className="max-w-xs whitespace-normal">{item.answer ?? "Resposta não concluída"}</td><td className="whitespace-nowrap">{formatDuration(item.durationMilliseconds)}</td><td>{item.status}</td></tr>)}</tbody></table></div></section>
  </div>;
}
