...
function customerPopup(customer: LogisticsMapCustomer): string {
  ...
  const location = customer.locationPrecision === "ADDRESS" && customer.address
    ? `<span><b>Endereço:</b> ${escapeHtml(customer.address)}</span><span><b>Precisão:</b> endereço geocodificado</span>`
...
function createCustomerPinIcon(customer: LogisticsMapCustomer): L.DivIcon {
  ...
  const precisionClass = customer.locationPrecision === "ADDRESS" ? "exact" : "municipality";
  const precisionLabel = customer.locationPrecision === "ADDRESS" ? "E" : "~";
  return L.divIcon({
...
export function LogisticsRegionMap({ customers, routes, periodDays = 30, compact = false }: LogisticsRegionMapProps) {
  ...
  useEffect(() => {
    ...
    for (const customer of filtered) {
      ...
      const locationTooltip = customer.locationPrecision === "ADDRESS" && customer.address
        ? customer.address
