...
describe("sales revenue and expense chart", () => {
  it("agrega os sete dias incluindo o limite inicial e final", () => {
    const series = buildSalesRevenueExpenseSeries(records, "daily", "2026-06-24");

    expect(series).toHaveLength(7);
    expect(series[0].period).toBe("2026-06-18");
  ...
  it("agrupa por mês, preenche ausência com zero e mantém soma das partes", () => {
    ...
    expect(series).toHaveLength(7);
    expect(series.map((point) => point.period)).toEqual(["2025-12", "2026-01", "2026-02", "2026-03", "2026-04", "2026-05", "2026-06"]);
  ...
  it("agrupa cinco anos, ignora datas inválidas e mantém valores não negativos", () => {
    const series = buildSalesRevenueExpenseSeries(records, "yearly", "2026-06-24");

    expect(series.map((point) => point.period)).toEqual(["2022", "2023", "2024", "2025", "2026"]);
