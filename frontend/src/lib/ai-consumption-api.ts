...
export type AiConsumptionReport = {
  ...
  byUser: Array<{ userId: number; userName: string; totalTokens: number; estimatedCostUsd: number; calls: number }>;
...
export type AiConsumptionConfiguration = {
  ...
  users: Array<{ userId: number; name: string; email: string; role: string; monthlyTokenLimit: number | null; alertPercentage: number | null }>;
};
...
export const getAiConsumptionReport = (from: string, to: string, userId?: string) => api<AiConsumptionReport>(`/report?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}${userId ? `&userId=${userId}` : ""}`);
