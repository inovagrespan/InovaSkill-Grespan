...
describe("route optimization UI", () => {
  ...
  it("expõe sugestão global pré-processada e consulta por rota usando a data selecionada", () => {
    ...
    expect(routePage).not.toContain("Ver recomendação");
    expect(routePage).not.toContain("RouteOptimizationDialog");
    expect(routePage).not.toContain("openOptimization(r)");
    expect(routePage).not.toContain("openOptimization(selectedRoute)");
