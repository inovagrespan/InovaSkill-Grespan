...
public sealed class LogisticsMapControllerTests
{
    ...
    public async Task Customers_ReturnsCurrentCustomersWithResolvedCoordinatesAndPendingCount()
    {
        ...
        Assert.Contains("\"visible\":0", json);
        Assert.Contains("\"withoutCoordinates\":2", json);
        Assert.DoesNotContain("Padaria Real", json);
        Assert.DoesNotContain("\"locationPrecision\":\"MUNICIPALITY\"", json);
        Assert.DoesNotContain("Mercado Pendente", json);
    }

    [Fact]
    public async Task Customers_DoesNotExposeInterpolatedAddressCoordinate()
    {
        await using var db = new ImportDbContext(new DbContextOptionsBuilder<ImportDbContext>()
        ...
        Assert.DoesNotContain("Cliente interpolado", json);
    }
